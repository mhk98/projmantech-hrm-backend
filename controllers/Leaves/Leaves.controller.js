...
module.exports.applyLeave = async (req, res) => {
    try {
        ...
        if (moment(startDate).isAfter(moment(endDate))) {
            ...
            return res.status(200).json({ "error": "Start date must be before end date" })
        }
        ...
        if (leaveType === 'annualLeave') {
            ...
            if (maxMonthLeavedays >= ApplicableLeaveDays) {
                ...
                if ((remainingLeaves - ApplicableLeaveDays) < 0) {
                    leaveApplicationStatus = "you have taken more leaves than reserved leaves for application select unpaidLeave";
                    return res.status(200).json({ "error": leaveApplicationStatus });
                }
                ...
                console.log(" Leave Reason-----------------------------", leaveObject);
                //Leave_Reason
                ...
                try {
                    ...
                    if (!leaveInsert) {
                        return res.status(200).json({ "error": "insert error for leave, Please try again" });
                    }
            ...
            leaveObject.addEmployeeEmployeeId = addEmployeeEmployeeId;
            leaveObject.Leave_Reason = Leave_Reason;
            console.log(" Leave Reason-----------------unpaid------------", leaveObject.Leave_Reason);
            //creating the leave instance for this req
            try {
                const leaveInsert = await Leave.create(leaveObject);
                if (!leaveInsert) {
...
async function dateArray(startDate, endDate) {
    ...
    while (startDate <= endDate) { // loop through each date between the start and end dates
...
module.exports.approveLeave = async (req, res) => {
    try {
        ...
        if (leave.leaveType === 'annualLeave') {
            leave.remainingLeaves = leave.remainingLeaves - leave.CountedDaysOfTakenLeave
        }
        leave.status = 'approved';
        await leave.save();
...
module.exports.allLeaves = async (req, res) => {
    try {
        const leave = await sequelize.query("select l.*, a.Employee_FirstName ,a.Employee_LastName  from leaves l , add_employees a where l.addEmployeeEmployeeId = a.Employee_Id ;", {
...
module.exports.allLeavecount = async (req, res) => {
    try {
        startDate = new Date();
        endDate = new Date();
        const leave = await sequelize.query("SELECT COUNT(Leave_Id) as leaves FROM Leaves WHERE startDate >= ? AND endDate <= ?;", {
...
module.exports.LeaveCount = async (req, res) => {
    try {
        const leaveCount = await Leave.count({ where: { status: 'pending' } }).then((leave) => {
            console.log("leave", leave);
            // if (!task) {
...
module.exports.fromtodateLeave = async (req, res) => {
    try {
        ...
        console.log("the startDate from req", req.body.startDate, 'the endDate from req', req.body.endDate);
        const leave = await sequelize.query("SELECT * FROM Leaves WHERE endDate BETWEEN ? AND ?;", {
            replacements: [startDate, endDate],
            type: sequelize.QueryTypes.SELECT
        })
        res.status(200).send(leave);
