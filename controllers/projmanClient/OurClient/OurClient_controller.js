...
module.exports.updateOurClient = async (req, res) => {
    try {
        const { id } = req.params;
        const isthere = await OurClients.findAll({ where: { Content_Id: id } });

        if (!isthere) {
            return res.send('Contant not found')

        }
        const result = await OurClient.update(req.body, {
            where: { Content_Id: id },
        });
        res.status(200).send({
            status: "Success",
...
module.exports.delete_OurClient = async (req, res) => {
    try {
        ...
        const isthere = await OurClient.findAll({ where: { Content_Id: id } });

        if (!isthere) {
            return res.send('Contant not found')

        }
        const result = await OurClient.destroy({ where: { Content_Id: id } })
